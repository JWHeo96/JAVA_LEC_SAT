...
public class ArrayCopyExample {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		for(int i=0; i<stdScores.length; i++) {
			System.out.println((i+1) + "번째 학생 점수 : " +stdScores[i] + " 점 " );
		}
		System.out.println("");
		// 오늘 한명이 전학을 와서, 그놈도 시험을 봤다.
		// 80점을 받았다.
		// stdScores <-- 학생 방이 5개 밖에 없다.
		// 앞으로 늘어날 수도 있으니, stdScores2 라는 10개의 방이 있는 집을 만들어서 이사가자.
		...
		for(int x = 0; x < stdScores.length + 1; x++) {
			System.out.println((x+1) + "번째 학생 점수 : " +stdScores2[x] + " 점 " );	
		}
		...
		*/
		...
		// B.System.arrayCopy()		
		// 학생 3명이 있는 학교 schoolA 에 학생 '괭이'가 전학을 왔어요.
		...
		schoolBStds[schoolAStds.length] = "괭이";

		Thread.sleep(1000);
		System.out.println("\n##### 이사완료 #####");
		for(int x = 0; x< schoolBStds.length; x++) {
			//if((String)schoolBStds[x] == null) break;
			System.out.println((x+1) + " 번째 학생 " + schoolBStds[x]);
		}
		...
		for(int x = 0; x< schoolCStds.length; x++) {
			//if((String)schoolBStds[x] == null) break;
			System.out.println((x+1) + " 번째 학생 " + schoolCStds[x]);
		}
